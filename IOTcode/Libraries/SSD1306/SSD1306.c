...
/**
  * @brief  Specifies the position where to display on the screen
  *         
...
void ssd1306_specify_display_position(uint8_t chXpos, uint8_t chYpos) 
{ 
	ssd1306_write_byte(0xb0 + chYpos, SSD1306_CMD);
	ssd1306_write_byte(((chXpos & 0xf0) >> 4) | 0x10, SSD1306_CMD);
	ssd1306_write_byte((chXpos & 0x0f) | 0x01, SSD1306_CMD); 
}   	  
...
/**
  ...
  * @retval  None
**/
void ssd1306_display_string(uint8_t chXpos, uint8_t chYpos, const uint8_t *pchString, uint8_t chSize)
{
#define MAX_CHAR_POSX 122
#define MAX_CHAR_POSY 58
	
    while (*pchString != '\0') {       
        if (chXpos > MAX_CHAR_POSX) {
			chXpos = 0;
			chYpos += 16;
		}
		
        if (chYpos > MAX_CHAR_POSY) {
			chYpos = chXpos = 0;
			ssd1306_clear_screen(0x00);
		}
		
        ssd1306_display_char(chXpos, chYpos, *pchString, chSize, 1);	 
        chXpos += 8;
        pchString ++;
    } 
}
